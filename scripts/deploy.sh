...
aws apigateway create-deployment --rest-api-id  $(aws cloudformation  describe-stacks --stack-name $STACK_NAME --query 'Stacks[0].Outputs[?OutputKey==`RestApiIddev`].OutputValue' --output text) --stage-name default --description 'Deployment to default stage'
